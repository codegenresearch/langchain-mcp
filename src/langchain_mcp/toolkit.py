...
class MCPToolkit(BaseToolkit):
    ...
    session: ClientSession = Field(..., description="The MCP session used to obtain the tools")
    ...
    async def get_tools(self) -> list[BaseTool]:
        if self._tools is None:
            raise RuntimeError("Toolkit not initialized. Call `initialize` method first.")
        return [
            MCPTool(
                toolkit=self,
